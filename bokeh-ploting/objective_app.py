...
# Loading results.
# We use pickle to load results.
pickle_in = open("result_cat.pickle","rb")
result = pickle.load(pickle_in)

#   Globals:
x_eval_selectors=[] # This list holds the sliders and selectors for parameter values
old_active_list = [] # Used to determine what x_eval_selectors were present before last update
source = {'reds' : []} # 'reds' is a (NxN) list. The diagonal holds a Span 
...
# 'samples"
x_eval_selectors_values = copy.copy(result.x) # We define the slider values globally so we can acces slider values for plots that have been destroyed

max_pars = len(result.space.dimensions) #Defines the maximum number of parameters that can be plotted
...
def get_plot_list(layout,result,active_list,n_points,x_eval):
    ...
    # red_vals is the coordinates for the x_eval that is mapped to integers in case they are 
    # categorical.
