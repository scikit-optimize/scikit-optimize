...
- :math:`f` is a black box for which no closed form is known (nor its gradients);
...
1. Given observations :math:`(x_i, y_i=f(x_i))` for :math:`i=1:t`, build a probabilistic model for the objective :math:`f`. Integrate out all possible true functions, using Gaussian process regression.
...
Acquisition functions :math:`u(x)` specify which sample x should be tried next:

- Expected improvement (default): :math:`-EI(x) = -\mathbb{E} [f(x) - f(x_t^+)]`
...
#############################################################################
# **Note.** In `skopt`, functions :math:`f` are assumed to take as input a 1D vector $x$ represented as an array-like and to return a scalar $f(x)$.
