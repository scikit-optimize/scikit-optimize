        ...
        * `points`: [list of lists, shape=(n_points, n_dims)]
        ...
        while len(rows) < n_samples:  # We keep sampling until all samples a valid with regard to the constraints
            ...
            if n_samples_candidates > 100000 and len(rows) < 100:
                ...
                raise RuntimeError(
                    'Could not find valid samples in constrained space. Please check that the constraints allows for valid samples to be drawn')
...
class Single:
        ...
        * `dimension_type` [str]:
        ...
        if dimension_type == 'categorical':
            if not (type(value) == int or type(value) == str or type(value) == float):
                raise TypeError(
                    'Single categorical constraint must be of type int, float or str. Got {}'.format(type(value)))
            ...
            raise ValueError(
                '`dimension_type` must be a string containing "categorical", "integer" or "real". got {}'.format(
                    dimension_type))
    ...
    def __repr__(self):
        return "Single(dimension={}, value={}, dimension_type={})".format(self.dimension, self.value,
                                                                          self.dimension_type)
...
class Bound_constraint():
    def __init__(self, dimension, bounds, dimension_type):
        ...
        if not dimension_type == 'categorical':
            if not len(bounds) == 2:
                raise ValueError(
                    'Length of bounds must be 2 for non-categorical constraints. Got {}'.format(len(bounds)))
        for bound in bounds:
...
class Inclusive(Bound_constraint):
    ...
    def __repr__(self):
        return "Inclusive(dimension={}, bounds={}, dimension_type={})".format(self.dimension, self.bounds,
                                                                              self.dimension_type)

    def __eq__(self, other):
        if isinstance(other, Inclusive):
            return all([a == b for a, b in zip(self.bounds,
                                               other.bounds)]) and self.dimension == other.dimension and self.dimension_type == other.dimension_type
        else:
            return False
...
class Exclusive(Bound_constraint):
    ...
    def __repr__(self):
        return "Exclusive(dimension={}, bounds={}, dimension_type={})".format(self.dimension, self.bounds,
                                                                              self.dimension_type)
    ...
    def __eq__(self, other):
        if isinstance(other, Sum):
            return all([a == b for a, b in zip(self.dimensions,
                                               other.dimensions)]) and self.value == other.value and self.less_than == other.less_than
        else:
            return False
        ...
        if isinstance(constraint, Single):
            ...
            for ind_dim in constraint.dimensions:
                if isinstance(space.dimensions[ind_dim], Categorical):
                    raise ValueError('Sum constraint can not be applid to categorical dimension: {}'.format(
            ...
            raise TypeError(
                'Constraints must be of type "Single", "Exlusive", "Inclusive", "Sum" or "Conditional". Got {}'.format(
                    type(constraint)))
...
def check_dim_and_space(space, constraint):
    ...
    if isinstance(space_dim, Real):
        if not constraint.dimension_type == 'real':
            raise TypeError('Constraint for real dimension {} must be of dimension_type real. Got {}'.format(ind_dim,
                                                                                                             constraint.dimension_type))
    elif isinstance(space_dim, Integer):
        if not constraint.dimension_type == 'integer':
            raise TypeError(
                'Constraint for integer dimension {} must be of dimension_type integer. Got {}'.format(ind_dim,
                                                                                                       constraint.dimension_type))
    elif isinstance(space_dim, Categorical):
        if not constraint.dimension_type == 'categorical':
            raise TypeError(
                'Constraint for categorical dimension {} must be of dimension_type categorical. Got {}'.format(ind_dim,
                                                                                                               constraint.dimension_type))
    else:
    ...
    if isinstance(dim, Real) or isinstance(dim, Integer):
        ...
        for value in bounds:
            if value not in dim.categories:
                raise ValueError(
                    'Categorical value {} is not in space with categories {}'.format(value, dim.categories))
...
def check_is_constraint(constraint):
    ...
    if not (isinstance(constraint, Single) or isinstance(constraint, Inclusive)
        ...
        raise TypeError('Constraint must be of type Inclusive, Exlusive, Single, Sum or Conditional. Got {}'.format(
            type(constraint)))
