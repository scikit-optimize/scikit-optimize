...
class Constraints:
        ...
        * `space` [Space]:
        ...
        for constraint in constraints_list:
            if isinstance(constraint, Single):
                ...
                raise TypeError('Constraint type {} is not'
                                'recognized'.format(type(constraint)))
    ...
    def rvs(self, n_samples=1, random_state=None):
        """Draw random samples that all are valid with regards to the
        constraints.
        ...
        # We keep sampling until all samples a valid with
        ...
        while len(rows) < n_samples:
            ...
            for i in range(self.space.n_dims):
                ...
                # If a dimension has a "Single"-type constraint we
                # just sample that value
                if self.single[i]:
            ...
            if n_samples_candidates > 100000 and len(rows) < 100:
                # If we have less than a 1/10.000 success rate on the
                # sampling we throw an error
                raise RuntimeError(
                    'Could not find valid samples in constrained space.'
                    'Please check that the constraints allows for valid'
                    'samples to be drawn')

        # We draw more samples when needed so we only
        # return n_samples of the samples
        return rows[:n_samples]

    def validate_sample(self, sample):
        """ Validates a sample of parameter values in
        regards to the constraints.
        ...
        # We iterate through all the dimensions and check
        # the the type of constriants that are applied
        # to a single dimensions, i.e Single, Exlcusive and Inclusive
        # We iterate through all samples which corresponds to number
        # of dimensions.
        for dim in range(len(sample)):
            ...
            if self.inclusive[dim]:
                # We go through all inlcusive constraints for this
                # dimension and if the value is not found to be included in
                # any of the bounds of the inclusive constraints we
                # return false.
            ...
            for constraint in self.exclusive[dim]:
                # The first time a value is inside of the excluded bounds
                # of the exclusive constraint we return false.
        ...
        for constraint in self.conditional:
            if not constraint.validate_sample(sample):
                return False
        # If we we did not find any violation of the constraints
        # we return True.
        return True
...
class Single:
    def __init__(self, dimension, value, dimension_type):
        ...
        This constraint enforces that all values drawn for the
        specified dimension equals `value`
        ...
        if dimension_type == 'categorical':
            if not (type(value) == int or type(value) == str or
                    type(value) == float):
                raise TypeError(
                    'Single categorical constraint must be of type'
                    'int, float or str. Got {}'.format(type(value)))
        elif dimension_type == 'integer':
            if not type(value) == int:
                raise TypeError('Single integer constraint must be of'
                                'type int. Got {}'.format(type(value)))
        elif dimension_type == 'real':
            if not type(value) == float:
                raise TypeError('Single real constraint must be of type'
                                'float. Got {}'.format(type(value)))
        else:
            raise ValueError(
                '`dimension_type` must be a string containing "categorical",'
                '"integer" or "real". got {}'.format(
                    dimension_type))
        if not (type(dimension) == int):
            raise TypeError('Constraint dimension must be of type int.'
                            'Got type {}'.format(type(dimension)))
        if not dimension >= 0:
    ...
    def __repr__(self):
        return "Single(dimension={}, value={}," \
               "dimension_type={})".format(self.dimension, self.value,
                                           self.dimension_type)

    def __eq__(self, other):
        if isinstance(other, Single):
            return self.value == other.value and self.dimension ==\
                   other.dimension and self.dimension_type ==\
                   other.dimension_type
        else:
            return False
...
class Bound_constraint():
    def __init__(self, dimension, bounds, dimension_type):
        ...
        if not type(bounds) == tuple and not type(bounds) == list:
            raise TypeError('Bounds should be a tuple or a list.'
        ...
        if not dimension_type == 'categorical':
            if not len(bounds) == 2:
                raise ValueError(
                    'Length of bounds must be 2 for non-categorical'
                    'constraints. Got {}'.format(len(bounds)))
        for bound in bounds:
            if dimension_type == 'integer':
                if not type(bound) == int:
                    raise TypeError('Bounds must be of type int for'
                                    'integer dimension.'
                                    'Got {}'.format(type(bound)))
            elif dimension_type == 'real':
                if not type(bound) == float:
                    raise TypeError('Bounds must be of type float'
                                    'for real dimension.'
                                    'Got {}'.format(type(bound)))
        if not (dimension_type == 'categorical' or dimension_type ==
                'integer' or dimension_type == 'real'):
            raise ValueError(
                '`dimension_type` must be a string containing "categorical",'
                '"integer" or "real". Got {}'.format(
                    dimension_type))
...
class Inclusive(Bound_constraint):
    def __init__(self, dimension, bounds, dimension_type):
        ...
        dimension are inside the bounds,
        with the bound values included.
        In case of categorical dimensions the constraint enforces that
        only values specified in bounds can be drawn.
        ...
        bounds : tuple
            ...
            For 'integer' dimensions the tuple must be of length 2 and
            only consist of floats.
    ...
    def __repr__(self):
        return "Inclusive(dimension={}, bounds={}," \
               "dimension_type={})".format(self.dimension, self.bounds,
                                           self.dimension_type)

    def __eq__(self, other):
        if isinstance(other, Inclusive):
            return all([a == b for a, b in zip(self.bounds,
                                               other.bounds)])\
                   and self.dimension ==\
                   other.dimension and self.dimension_type ==\
                   other.dimension_type
...
class Exclusive(Bound_constraint):
    def __init__(self, dimension, bounds, dimension_type):
        ...
        This constraint enforces that all values drawn for the
        specified dimension are outside the bounds,
        with the bound values excluded.
        In case of categorical dimensions the constraint enforces
        that values specified in bounds can not be drawn.
        ...
        bounds : tuple
            The bounds for the constraint.
            For 'real' dimensions the tuple must be of length 2 and
    ...
    def __repr__(self):
        return "Exclusive(dimension={}, bounds={}," \
               "dimension_type={})".format(self.dimension, self.bounds,
                                           self.dimension_type)
...
class Sum():
    def __init__(self, dimensions, value, less_than=True):
        ...
        specified dimensions, is less_than or equal to a value.
        Can only be used with integer or real dimensions.

        Parameters
        ----------
        dimensions : list of ints
            A list of integers coresponding to the index of the
            dimensions that should be summed

        value : float or int
            The value for which the sum should be less than or equal to.

        less_than : bool, default=True
            If true then the sum should be less than or equal to the value.
            If False the sum should
            be greater than or equal to the value.
        """

        if not (type(dimensions) == tuple or type(dimensions) == list):
            raise TypeError('Argument `dimensions` must be of type tuple or'
                            'list. Got {}'.format(type(dimensions)))
        if not len(dimensions) > 1:
            raise ValueError('Argument `dimensions` must have a lenght of'
                             'more than 1. Got {}'.format(len(dimensions)))
        ...
        if not (type(value) == int or type(value) == float):
            raise TypeError('Argument `value` must be of type float or int.'
                            'Got {}'.format(type(value)))
        if not type(less_than) == bool:
            raise TypeError('Argument `less_than` must be of type bool'
                            '(True or False). Got {}'.format(type(less_than)))
    ...
    def __repr__(self):
        return "Sum(dimensions={}, value={}," \
               "less_than={})".format(self.dimensions, self.value, self.less_than)

    def __eq__(self, other):
        if isinstance(other, Sum):
            return all([a == b for a, b in zip(self.dimensions,
                                               other.dimensions)])\
                   and self.value == other.value and self.less_than ==\
                   other.less_than
        else:
            return False
...
class Conditional():
    def __init__(self, condition, if_true=None, if_false=None):
        ...
        This constraint enforces other constraints depending on
        the 'condition'.
        If condition is satisfied the if_true constraint is applied.
        If not then the if_false
        ...
        if_true : Constraint or tuple/list of constraints, default=None
            If the constraint from the condition-argument is
            satisfied then this constraint is applied.

        if_false : Constraint or tuple/list of constraints, default=None
            If the constraint from the condition-argument is not
            satisfied then this constraint is applied.
        """
    ...
    def __repr__(self):
        return "Conditional(condition={}, if_true={}," \
               "if_false={})".format(
            self.condition, self.if_true, self.if_false)

    def __eq__(self, other):
        if isinstance(other, Conditional):
            return self.condition == other.condition and self.if_true ==\
                   other.if_true and self.if_false == other.if_false
        else:
            return False


def check_constraints(space, constraints):
    """ Checks if list of constraints is valid when
    compared with the dimensions of space.
    ...
    if not type(constraints) == list:
        raise TypeError('Constraints must be a list of'
                        'constraints. Got {}'.format(type(constraints)))
    n_dims = space.n_dims

    single_constraints = [False] * n_dims
    for constraint in constraints:

        # Check if constraints are inside bounds of space and
        # if more than one single constraint has been
        ...
        if isinstance(constraint, Single):
            ...
            if single_constraints[ind_dim] == True:
                raise IndexError('Can not add more than one Singe-type'
                                 'constraint to dimension {}'.format(ind_dim))
            single_constraints[ind_dim] = True
            space_dim = space.dimensions[constraint.dimension]
            check_value(space_dim, constraint.value)
        elif isinstance(constraint, Inclusive)\
                or isinstance(constraint, Exclusive):
            check_dim_and_space(space, constraint)
            space_dim = space.dimensions[constraint.dimension]
            check_bounds(space_dim, constraint.bounds)
        elif isinstance(constraint, Sum):
            if not all(dim < n_dims for dim in constraint.dimensions):
                raise IndexError('Dimension index exceeds'
                                 'number of dimensions')
            for ind_dim in constraint.dimensions:
                if isinstance(space.dimensions[ind_dim], Categorical):
                    raise ValueError('Sum constraint can not be applid'
                                     'to categorical dimension: {}'.format(
                        space.dimensions[ind_dim]))
        elif isinstance(constraint, Conditional):
            # We run check_constraints on each constraint
            # instance in the conditional constraint.
            ...
            raise TypeError(
                'Constraints must be of type "Single", "Exlusive",'
                '"Inclusive", "Sum" or "Conditional". Got {}'.format(
                    type(constraint)))
...
def check_dim_and_space(space, constraint):
    ...
    if not ind_dim < n_dims:
        raise IndexError('Dimension index {} out of'
                         'range for n_dims = {}'.format(ind_dim, n_dims))
    space_dim = space.dimensions[constraint.dimension]
    ...
    if isinstance(space_dim, Real):
        if not constraint.dimension_type == 'real':
            raise TypeError('Constraint for real dimension {} must be of'
                            'dimension_type real. '
                            'Got {}'.format(ind_dim,
                                            constraint.dimension_type))
    elif isinstance(space_dim, Integer):
        if not constraint.dimension_type == 'integer':
            raise TypeError(
                'Constraint for integer dimension {} must be of'
                'dimension_type integer.'
                'Got {}'.format(ind_dim, constraint.dimension_type))
    elif isinstance(space_dim, Categorical):
        if not constraint.dimension_type == 'categorical':
            raise TypeError(
                'Constraint for categorical dimension {} must be of'
    ...
    if isinstance(dim, Real) or isinstance(dim, Integer):
        for value in bounds:
            if value < dim.low or value > dim.high:
                raise ValueError('Bounds {} exceeds bounds of'
                                 'space {}'.format(
                    bounds, [dim.low, dim.high]))
    else:  # Categorical dimensions.
        for value in bounds:
            if value not in dim.categories:
                raise ValueError(
                    'Categorical value {} is not in space with'
                    'categories {}'.format(value, dim.categories))
...
def check_value(dim, value):
    ...
    if isinstance(dim, Real) or isinstance(dim, Integer):
        if value < dim.low or value > dim.high:
            raise ValueError('Value {} exceeds bound'
                             'of space {}'.format(value, [dim.low, dim.high]))
    else:  # Categorical dimension.
        if value not in dim.categories:
            raise ValueError('Categorical value {} is not in'
                             'space with categoreis {}'.format(
                value, dim.categories))


def check_is_constraint(constraint):
    """Checks if constraint is a valid constraint type.
    Throws error otherwise.
    Types can be Single, Inclusive, Exclusive, Sum and Conditional
    """

    if not (isinstance(constraint, Single)
            or isinstance(constraint, Inclusive)
            or isinstance(constraint, Exclusive)
            or isinstance(constraint, Sum)
            or isinstance(constraint, Conditional)):
        raise TypeError('Constraint must be of type Inclusive,'
                        'Exlusive, Single, Sum or Conditional.'
                        'Got {}'.format(type(constraint)))
