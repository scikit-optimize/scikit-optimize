...
def gbrt_minimize(func, dimensions, base_estimator=None,
    ...
    following. If ``x0`` is provided but not ``y0``, then the elements of
    ``x0`` are first evaluated, followed by ``n_random_starts`` evaluations.
    ...
    acq_func : string, optional (default="LCB")
        ...
        - "gp_hedge" Probabilistically choose one of the above three
          ...
          given to these gains can be set by ``\eta`` through
          ``acq_func_kwargs``.
    ...
    callback : callable, list of callables, optional
        If callable then ``callback(res)`` is called after each call to
        ``func``. If list of callables, then each callable in the list is
        called.

    n_points : int, optional (default=10000)
    ...
    n_jobs : int, optional (default=1)
        Number of cores used to parallize fitting of ``base_estimator``.
