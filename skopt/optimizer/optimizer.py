...
class Optimizer(object):
    ...
    optimisers provided by scikit-optimize use this class under the hood.
    ...
    base_estimator : {'GP', 'RF', 'ET', 'GBRT', sklearn regressor}, optional (default='GP')
        If not a string then should inherit from
        ``sklearn.base.RegressorMixin``. In addition the ``predict`` method,
        should have an optional ``return_std`` argument, which returns
    ...
    acq_func : string, optional (default='gp_hedge')
        ...
        - 'gp_hedge' Probabilistically choose one of the above three
          acquisition functions at every iteration.
            ...
            - At every iteration,
                ...
                - Out of all these candidate points, the next point ``X_best``
                  is chosen by ``softmax(\eta g_i)``
    ...
    acq_optimizer : {'sampling', 'lbfgs'}, optional (default='auto')
    ...
    space : skopt.space.Space
        An instance of ``skopt.space.Space``. Stores parameter search space
        used to sample points, bounds, and type of parameters.
    ...
    def ask(self, n_points=None, strategy="cl_min"):
        ...
        strategy : string, optional (default='cl_min')
    ...
    def tell(self, x, y, fit=True):
        ...
        ``ask()`` or other points. By default a new model will be
        fit to all observations. The new model is used to suggest the next
        point at which to evaluate the objective. This point can be retrieved
        by calling ``ask()``.
