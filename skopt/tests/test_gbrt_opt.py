...
def check_minimize(func, y_opt, dimensions, margin, maxiter):
    r = gbrt_minimize(func, dimensions, maxiter=maxiter, random_state=1)
    assert_less(r.fun, y_opt + margin)


def test_gbrt_minimize():
    yield (check_minimize, bench1, 0., [(-2.0, 2.0)], 0.05, 75)
    yield (check_minimize, bench2, -5, [(-6.0, 6.0)], 0.05, 75)
    yield (check_minimize, bench3, -0.9, [(-2.0, 2.0)], 0.05, 75)
    yield (check_minimize, branin, 0.39, [(-5.0, 10.0), (0.0, 15.0)],
           0.1, 100)
    yield (check_minimize, hart6, -3.32, np.tile((0.0, 1.0), (6.0, 1.0)),
           1.0, 200)
