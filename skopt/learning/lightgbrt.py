...
class LGBMQuantileRegressor(BaseEstimator, RegressorMixin):
    ...
    This is a wrapper around `LGBMRegressor`'s quantile
    ...
    base_estimator : LGBMRegressor instance or None (default)
    ...
    def fit(self, X, y):
        ...
        if self.base_estimator is None:
            base_estimator = LGBMRegressor(objective='quantile')
        else:
            ...
            if not base_estimator.get_params()['objective'] == 'quantile':
                raise ValueError('base_estimator has to use quantile'
                                 ' loss not %s' % base_estimator.get_params()['objective'])
        ...
        # base_estimator.set_params(random_state=42)
        regressors = []
