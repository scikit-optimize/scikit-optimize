...
@pytest.mark.fast_test
def test_lightgbrt_base_estimator():
    rng = np.random.RandomState(1)
    N = 10000
    X = np.ones((N, 1))
    y = rng.normal(size=N)
    ...
    rgr = LGBMQuantileRegressor(base_estimator=base)
    ...
    base = LGBMRegressor(objective='quantile')
    rgr = LGBMQuantileRegressor(base_estimator=base)
    rgr.fit(X, y)
...
@pytest.mark.fast_test
def test_lightgbrt_in_parallel():
