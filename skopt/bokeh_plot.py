...
from skopt.plots import _map_categories, partial_dependence
...
# Selectors: Sliders that appear on top of the plots with which the red
# markers can be adjusted
# Toggle_x button group: A group of buttons where we can toggle what
# parameters we want to plot.
# Generate button: Dependence is only calculated a new when this buton
# is pressed
...
# Y values: A small text field which shows the expected value as well as
# confidence interval

# Here we define the global variables (global variables are generally not
# advices so maybe we should change this at some point)
result = None
x_eval_selectors = None  # This list holds the sliders and selectors for
# parameter values
...
source = {'reds': []}
# object with a "location" value, that detemines the location for the red line
# in the 1d plots.
# The off diagonals holds coordinates for the red markers used in the contour
# plot.
# 'samples"
x_eval_selectors_values = None
max_pars = None  # Defines the maximum number of parameters that can be
# plotted
layout = None
...
def set_globals(parsed_result):
    global button_generate, buttons_toggle_x, dropdown_eval_method,\
        slider_n_points, max_pars, source, old_active_list
    global result, x_eval_selectors, layout, x_eval_selectors_values,\
        button_partial_dependence, y_value, button_color_map, colorbar,\
    ...
    dropdown_eval_method = Select(title="Evaluation method:", value='Result',
                                  options=[
                                  'Result', 'Exp min',
                                      'Exp min rand', 'Sliders'],
                                  width=200, height=40)
    ...
    colorbar = Div()  # We initiate colorbar as an empty div first and then
    # change it to a plot in case we want to show it
    row_plots = row([])
...
def handle_button_generate(layout, result):
    ...
    # Remove plots when button is clicked. If we dont do this we ge the
    # "Cant apply patch" bug.
    ...
    if active_list:  # Only plot if there is at least one selection
        ...
        y, std = result.models[-1].predict(x_eval_transformed,
                                           return_std=True)
        ...
        if colorbar_interval:
            colorbar_plot = figure(plot_height=200, plot_width=100,
                                   tools='', x_range=[
                                   0, 1], y_range=colorbar_interval)
            im = get_colorbar_as_rgba()
            colorbar_plot.image_rgba(image=[
                                     im], x=0, y=colorbar_interval[0], dw=1,
        ...
        layout.children[0].children[1] = Div(
            text="""<font size="10"><br><br>Let's select som parameters to plot!</font>""",
            width=500, height=100)
...
def get_x_eval_selectors_values():
    # Returns the values for the x_eval_selectors. Uses
    # the global x_eval_selectors_values
    # and for each selector that is present in the GUI we replace
    # the corresponding value.
    # Even though a selector isnot present in the GUI the last value
    # that it had i still retrived.
...
def get_active_list():
    # returns the a list of the indexes of parameters that have been
    # toggled in
...
def get_use_partial_dependence():
    # Returns True or false depending on wether or not the partial
    # dependence button
...
def get_plot_list(layout, result, active_list, n_points, x_eval, confidence):
    # Returns a NxN list of plots where N is the number of parameters to
    # be plotted.
    ...
    # First we calculate all the data tat will beused for all the plots.
    # After this
    # we draw the plots.
    plots_data = []
    # It is important that we do it in this order as we need to know the
    # maxmimum
    # and minimum values before drawing the plots,
    ...
    # False otherwise.
    # red_vals are the coordinates for the x_eval that is mapped to integers in
    # case they are
    # categorical.
    ...
    # While iterating through each plot we keep track of the max and min values of y
    # and z in case of 2d plots.
    ...
    plots_to_do = len(active_list)*(len(active_list)+1)/2  # Triangular
    # numbers
    ...
    for i_active in range(len(active_list)):
        ...
        for j_active in range(len(active_list)):
            ...
            if j > i:  # We only plot the lower left half of the grid,
                # to avoid duplicates.
                break
            elif i == j:  # Diagonal
                # Passing j = None to dependence makes it calculate a diagonal plot
                xi, yi =partial_dependence(
                    space, model, i, j=None, sample_points=None, n_samples=250,
                    n_points=n_points, x_eval=dependence_eval)
                ...
                xi, yi, zi = partial_dependence(
                    space, model, i, j=j, sample_points=None, n_samples=50,
                    n_points=n_points, x_eval=dependence_eval)
            ...
            current_plot += 1
            # We put the progress into the div where we normally
            # show the y-values (confidence bounds)
    ...
    for i_active in range(len(active_list)):  # Only plot the
        # selected parameters
        source['reds'].append([])
        plots.append([])
        for j_active in range(len(active_list)):
            i = active_list[i_active]
            j = active_list[j_active]
            if j > i:  # We only plot the lower left half of the grid, to
                # avoid duplicates.
                break
            elif i == j:  # Diagonal
                # Passing j = None to dependence makes it calculate a
                # diagonal plot
                ...
                # Source red is what we end up appending to the global
                # source variable
                # So the location of the red line can be changed interactively
                source_red = Span(
                    location=red_vals[i], dimension='height',
                    line_color='red', line_width=3)
                #dotte_line_low = Span(location=red_vals[i],
                ...
                if use_same_color_map:
                    ...
                    cmap = None
                # Get an rgba contour image from matplotlib as bokeh does not
                # support contour plots
                im = get_plt_contour_as_rgba(
                    xi, yi, zi, confidence, cmap=cmap,
            ...
            if iscat[i] and i != j:  # In case of diagonal the y-labels are
                # numbers and therefore should not be rotated
...
def get_plots_layout(layout, result, active_list, n_points,
    ...
    # the layout can be regarded as a (N+1,N) grid to make room for the
    # top selector.
    # The top row therefore consists of only on selector and the bottom
    # row has no
...
def get_x_eval_selectors_list(result, active_list, x_eval):
    # Returns a list of selectors. The selectors are sliders for
    # numerical values and dropdown menus
    # ("Select" object) for categorical values. The selectors are
    # interactive with callbacks everytime
    # a changed by the user
    global x_eval_selectors_values

    bounds = result.space.bounds  # Used for defining what values
    # can be selected
    x_eval_selectors = []
    n = 0  # Index of the plots. Example: If only parameter 3 and 5
    # is being plotted
    # the selectors for these parameters still have index n = 0 and n= 1.
    for i in active_list:  # Only get selecters that is going to be shown
        # in GUI
        if isinstance(result.space.dimensions[i], Categorical):  # Categorical
            ...
            # This object gets a title equal to the parameter number, and
            # the value is set to x_eval
            select = Select(
                title='X'+str(i), value=x_eval[i], options=cats,
                width=200, height=15)
            # Here we define a callback that updates the appropiate red
            # markers by changing
            # with the current value of the selector by changing the global
            # "source" variable
            # The callback function is written in javascript
            select.js_on_change('value',
                                CustomJS(
                                    args=dict(source=source, n=n, cats=cats),
                                         code="""
            ...
            slider = Slider(
                start=start, end=end, value=x_eval[i], step=step,
                title='X'+str(i), width=200, height=30)
            # javascript callback function that gets called everytime
            # a user changes the slider value
...
def get_plt_contour_as_rgba(xi, yi, zi, confidence, cmap=None,
    ...
    # We create a matplotlib figure and draws it so we can capture
    ...
    if draw_confidence:
        ax.contour(xi, yi, zi, levels=[
                   confidence[1]], locator=None, colors='black',
...
def get_x_eval(result, active_list):
    # Returns the evaluation values that is defined by the
    # evaluationmethod dropdown menu
    _, _, iscat = _map_categories(result.space, [result.x], result.x)
    eval_method = dropdown_eval_method.value  # Get evaluation method
    # from GUI
    ...
    if eval_method == 'Result':
        ...
        x, _ = expected_minimum_random_sampling(
            result, n_random_starts=np.min([10**len(result.x), 10000]))
    else:
...
def get_step_size(start, end):
    # Returns the stepsize to be used for sliders the stepsize will always be
    # of the form 10**x
